...

{
  ...
  searchRoom.dateFilter = new FilterDateDropdown({parentSelector: "date-filter", initDate: ["19.08.2021", "23.08.2021"]})
  searchRoom.guests = new DropdownClass({ parentSelector: "guests-quantity"});
  searchRoom.convenience = new DropdownClass({parentSelector: "convenience-filter", items: {"спальни":  2, "кровати": 2 , "ванные комнаты": 0}, getListClass: (items) => {return new ListConvenience(items)}});
  searchRoom.priceFilter = new Slider({parentSelector: "price-filter", min: 0, max:16000, initValues: ["5000", "10000"]});

  searchRoom.filters = document.querySelector(".filters");
  searchRoom.filtersButton = document.querySelector(".filters__show-button");
